			...
			resize: function(options) {
				...
				try {
					...
					if (opts.crop) {
						...
						scale = Math.min(1,opts.width/self.width, opts.height/self.height);
					}
	...
	function HTML5Image() {
		...
		Basic.extend(this, {
			...
			resize: function(rect, ratio, options) {
				...
				// Some browsers will draw the image with the correct orientation (Firefox) and some don't (Safari, Chrome)
				// This tries to detect if the orientation is wrong so that we can reverse it
				var browserHasAlreadyRotated = ( this.meta && this.meta.exif && this.meta.exif.PixelXDimension == rect.height );

				// rotate if required, according to orientation tag
				if (!_preserveHeaders || browserHasAlreadyRotated) {
					var orientation = (this.meta && this.meta.tiff && this.meta.tiff.Orientation) || 1;
					if ( browserHasAlreadyRotated ) {
						switch ( orientation ) {
							case 6:
								orientation = 8;
								break;
