...
class _clubs extends \IPS\Api\Controller
{
	...
	public function GETindex()
	{
		...
		return new \IPS\Api\PaginatedResponse(
			200,
			\IPS\Member\Club::clubs( $this->member ?: NULL, array( ( $page - 1 ) * $perPage, $perPage ), 'created' ),
			$page,
			'IPS\Member\Club',
			\IPS\Member\Club::clubs( $this->member ?: NULL, array( ( $page - 1 ) * $perPage, $perPage ), 'created', FALSE, array(), NULL, TRUE ),
			$this->member,
			$perPage
		);
	}
